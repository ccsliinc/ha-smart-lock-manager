...
class SmartLockManagerLock:
    ...
    # Lock settings
    settings: LockSettings = field(default_factory=lambda: LockSettings(friendly_name="Smart Lock"))
    ...
    def set_code(
        ...
        _LOGGER.debug("Set code for %s slot %s: active=%s, pin_code=%s", 
                     self.lock_name, slot_number, slot.is_active, bool(pin_code))
    ...
    def get_active_codes_count(self) -> int:
        ...
        active_slots = [slot_num for slot_num, slot in self.code_slots.items() if slot.is_active]
    ...
    def get_valid_slots_now(self) -> Dict[int, CodeSlot]:
        ...
        return {num: slot for num, slot in self.code_slots.items() if slot.is_valid_now()}
    ...
    def check_and_update_slot_validity(self) -> List[int]:
        ...
        for slot_number, slot in self.code_slots.items():
            ...
            if should_disable and slot.is_active:
                ...
                _LOGGER.warning("Auto-disabled slot %s in %s: %s", slot_number, self.lock_name,
                              "expired" if slot.is_expired() else "max uses reached")
    ...
    def get_slots_needing_sync(self, zwave_codes: Dict[int, Dict] = None) -> Dict[str, List[int]]:
        """
    ...
    def resize_slots(self, new_slot_count: int) -> bool:
        ...
        # If increasing slots, add new empty slots
            ...
            for slot_num in range(self.start_from + old_count, self.start_from + new_slot_count):
                self.code_slots[slot_num] = CodeSlot(slot_number=slot_num)
    ...
    def disable_slot(self, slot_number: int) -> bool:
        ...
        self.code_slots[slot_number].is_active = False
        return True
    
    def check_and_update_slot_validity(self) -> List[int]:
        """Check all slots and disable any that should no longer be active.
        Returns list of slot numbers that were disabled."""
        disabled_slots = []
        
        for slot_num, slot in self.code_slots.items():
            if slot.is_active and slot.should_disable():
                slot.is_active = False
                disabled_slots.append(slot_num)
    ...
    def get_usage_statistics(self) -> Dict[str, any]:
        ...
        active_users = len([slot for slot in self.code_slots.values() if slot.is_active])
        most_used_slot = max(self.code_slots.values(), key=lambda s: s.use_count, default=None)
        ...
        return {
            ...
            "slots_with_limits": len([s for s in self.code_slots.values() if s.max_uses > 0]),
            "expired_slots": len([s for s in self.code_slots.values() if s.end_date and datetime.now() > s.end_date]),
        }
