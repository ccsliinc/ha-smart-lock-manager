...
class TestSmartLockManagerConfigFlow:
    ...
    async def test_async_step_user_form_submission(
        self, config_flow, mock_lock_entities
    ):
    ...
    async def test_form_validation_invalid_start_from(
        self, config_flow, mock_lock_entities
    ):
    ...
    async def test_options_flow_available(self, config_flow):
        """Test that the config flow exposes a working options flow.

        ``async_get_options_flow`` is a @staticmethod on the ConfigFlow class
        (HA's required location) that returns a SmartLockManagerOptionsFlow
        handler, so HA renders a "Configure" button for each entry.
        """
...
class TestConfigFlowEdgeCases:
    ...
    async def test_version_property(self, config_flow):
        ...
        assert hasattr(SmartLockManagerConfigFlow, "VERSION") or hasattr(
            config_flow, "VERSION"
        )
